...
[**Named graphs**](/v0.4/storage/named-graphs).
...
🚀 **v0.5 forward edge** — graph-lifecycle UDFs (`drop_graph`,
`clear_graph`, `copy_graph`, `move_graph`) are in the v0.4 cycle,
landing as a coherent group with SPARQL UPDATE. See
...
- [Named graphs (IRI ↔ id)](/v0.4/storage/named-graphs).
- [GRAPH `<iri> { … }` in SPARQL](/v0.4/query/graph-clause).
