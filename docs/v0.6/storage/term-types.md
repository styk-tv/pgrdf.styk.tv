---
title: Term types — typed literals, language tags, blank nodes, RDF lists
description: Turtle ingest preserves the full RDF term-type space — IRIs, plain and datatyped literals, language-tagged literals, blank nodes, and RDF collections.
---

# Term types — typed literals, language tags, blank nodes, RDF lists

> Turtle ingest preserves the full RDF term-type space: IRIs, plain
> literals, datatyped literals, language-tagged literals, blank nodes,
> and RDF collections.

## What it does

The Turtle parser round-trips every term species the standard defines:
...
Distinct literals that share a lexical value are kept distinct on their
**full identity** `(lexical_value, datatype, language)` — `"Berlin"@en`
and `"Berlin"@de` are separate dictionary terms, as are
`"1"^^xsd:integer` and `"1"`. The [staged loader](/v0.6/storage/staged-loader)
holds this exact-dedup contract at billion scale.

## Why you'd use it

- **Data scientists** — numeric or language-conditional FILTERs on real
  data work because the type is preserved at ingest time, not stripped
  to plain strings.
- **Ontologists** — load OWL, SHACL, PROV-O and similar vocabularies
  with confidence that the datatype assertions round-trip.
- **Project managers** — the on-disk representation matches the W3C
  spec; no lossy normalisation surprises downstream.
...
- [Hexastore + dictionary](/v0.6/storage/hexastore).
- [FILTER expressions](/v0.6/query/filter).
