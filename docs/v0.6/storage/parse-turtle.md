---
title: Inline Turtle / TriG / N-Quads ingest
description: pgrdf.parse_turtle / parse_trig / parse_nquads parse RDF directly from a SQL text literal — same parser family as load_turtle, no filesystem dependency.
---

# Inline Turtle / TriG / N-Quads ingest

> Same parser family as `load_turtle`, no filesystem dependency. Pass
> the RDF source as a SQL text literal — Turtle, TriG, or N-Quads.
...
count loaded. Useful when you don't want to land a file on the server
first — notebooks, orchestration code, prompt-driven synthetic graphs,
and test fixtures all benefit.

### Quad-bearing serialisations — TriG + N-Quads

For serialisations that carry the named-graph dimension themselves, two
sibling UDFs ship:
...
`parse_turtle`. `parse_trig` accepts the TriG `GRAPH <iri> { … }` block
syntax; `parse_nquads` accepts the line-based N-Quads form where the
optional fourth term names the graph. The `graph_id` argument is the
default/fallback graph for triples that carry no explicit graph in the
source.

## Why you'd use it

- **Project managers** — embed seed data for new tenants inline in
  migration scripts; no file deployment needed.
...
- [Load Turtle from disk](/v0.6/storage/load-turtle) — file path
  variant.
- [Bulk ingest](/v0.6/storage/bulk-ingest) — the loader family for
  large loads.
- [Verbose ingest statistics](/v0.6/storage/verbose-stats) — sibling
  `pgrdf.parse_turtle_verbose` returns JSONB.
