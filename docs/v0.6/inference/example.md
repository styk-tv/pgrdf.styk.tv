---
title: Worked example — subclass chain
description: A two-hop rdfs:subClassOf materialization you can copy-paste into psql — load, materialize, query, and inspect the inferred vs base breakdown.
---

# Worked example — subclass chain

A two-hop `rdfs:subClassOf` example you can copy-paste into psql and
watch run end-to-end.
...
Three things are stated explicitly. Without materialization, a query
asking "what types does Alice have?" returns only the one that was
asserted directly:
...
The reasoner emits 11 new triples. (The exact count varies with the
OWL 2 RL closure — it includes RDFS-style class declarations like
`ex:Person rdf:type rdfs:Class`, plus the transitive type
assertions.)
...
See [Idempotence + operator safety](/v0.6/inference/idempotence) for
the contract.
