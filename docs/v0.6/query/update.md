...
**routes through the SQL UDF surface** ([`drop_graph`](/v0.6/storage/drop-graph),
[`clear_graph`](/v0.6/storage/clear-graph),
[`add_graph(iri)`](/v0.6/storage/named-graphs)) — the SPARQL
...
| SPARQL | Routes to |
|---|---|
| `DROP GRAPH <iri>` | [`pgrdf.drop_graph(id, cascade => TRUE)`](/v0.6/storage/drop-graph) |
| `CLEAR GRAPH <iri>` | [`pgrdf.clear_graph(id)`](/v0.6/storage/clear-graph) |
| `CREATE GRAPH <iri>` | [`pgrdf.add_graph(iri TEXT)`](/v0.6/storage/named-graphs) |
...
- Each UPDATE fixture uses `setup.sql` instead of `data.ttl` —
  ...
  produce.
- Test bar at v0.6.14: **294 pgrx + 93 pg_regress + 51
  W3C-SPARQL + 25 W3C SHACL Core + 3 LUBM**.

## See also

- [BGP joins](/v0.6/query/bgp-joins) — read counterpart to
  `INSERT WHERE`.
- [`sparql_parse`](/v0.6/query/sparql-parse) — preview UPDATE
  shape and routing without execution.
- [GRAPH `<iri> { … }`](/v0.6/query/graph-clause) — the same
  scoping mechanic applies inside UPDATE templates / WHERE.
- [Graph lifecycle UDFs](/v0.6/storage/lifecycle) — the SQL
  UDFs the SPARQL `DROP / CLEAR / CREATE GRAPH` forms route
  through.
- [Forward edge — what's next](/v0.6/query/roadmap) — the
  complete SPARQL surface, the v0.6 forward plan, and the
