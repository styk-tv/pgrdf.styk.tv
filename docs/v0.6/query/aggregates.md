...
Aggregates also run over `UNION` — each branch sub-SELECTs the
aggregate / GROUP-BY variables' dictionary ids into a shared pool,
and the aggregate runs over the combined relation.
