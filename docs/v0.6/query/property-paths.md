...
> Full SPARQL 1.1 property paths — `^` `+` `*` `?` `|`. They
> compose for free with named-graph scoping, BGP joins,
> OPTIONAL/UNION/MINUS, CONSTRUCT, and UPDATE WHERE bodies,
> because path support is recognised at the single WHERE
...
**Out of scope:** negated property sets `!(…)` (panics with a
stable prefix). **Use a multi-pattern BGP for sequences** —
...
When [`pgrdf.materialize(graph_id)`](/v0.6/inference/) has
...
[inference](/v0.6/inference/) and query: materialise once, then
...
once — SELECT, ASK, [`pgrdf.construct`](/v0.6/query/construct),
and [SPARQL UPDATE](/v0.6/query/update) WHERE bodies. They join
to ordinary BGP patterns, [named-graph scoping](/v0.6/query/graph-clause),
and [OPTIONAL/UNION/MINUS](/v0.6/query/optional) with no
special-casing.
...
- [Inference](/v0.6/inference/) — materialise a closure once, then
  the no-CTE fast path makes `subClassOf+` a plain lookup.
- [GRAPH clause](/v0.6/query/graph-clause) — paths scope to a
  named graph for free.
- [`sparql_parse`](/v0.6/query/sparql-parse) — lowers the full
  executable path set; flags only the gated remainder.
- [Forward edge](/v0.6/query/roadmap) — what's still pre-v1.0.
