...
`conforms` is `true` if and only if `results` is empty, mirroring
the W3C `sh:conforms` invariant. A shapes graph that names no
targets is vacuously conforming; a missing graph (zero triples)
follows the same rule.

## Validation over the entailed closure

Because validation rehydrates both base **and** inferred rows from
the same dictionary-encoded tables, a `pgrdf.materialize` call
followed by `pgrdf.validate` validates the **entailed closure** —
shapes can assert constraints over triples that were inferred by
OWL 2 RL or RDFS, not only over the asserted base data.
...
[**Next — Worked example →**](/v0.6/validation/example)
