...
[`pgrdf.load_turtle_verbose`](/v0.5/storage/verbose-stats) — same
...
[shared-memory dictionary cache](/v0.5/storage/shmem-cache), and
flushes quads in batches using a prepared `INSERT` statement
([bulk ingest](/v0.5/storage/bulk-ingest)). The full pipeline is
described in
...
- [Inline Turtle ingest](/v0.5/storage/parse-turtle) — for fixture
